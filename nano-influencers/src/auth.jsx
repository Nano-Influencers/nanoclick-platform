...
export function LoginPage() {
  ...
  return <div className="auth-page"><div className="auth-card">
    ...
    <h1>Welcome back</h1><p className="auth-sub">Log in to manage your campaigns.</p>
...
export function RegisterPage() {
  ...
  return <div className="auth-page"><div className="auth-card">
    <div className="auth-brand"><span className="l-dot" />The Nano Influencers</div>
    <h1>Create your account</h1><p className="auth-sub">Set up campaigns and reach real people in minutes.</p>
    {error && <div className="auth-error">{error}</div>}
    <form onSubmit={submit}>
      <label>Full name<input className="field" required value={fields.full_name} onChange={set("full_name")} placeholder="Jane Doe" /></label>
      <label>Email<input className="field" type="email" required value={fields.email} onChange={set("email")} placeholder="you@example.com" /></label>
      <label>Password<input className="field" type="password" required minLength={8} value={fields.password} onChange={set("password")} placeholder="At least 8 characters" /></label>
      <label>Referral code <small className="auth-optional">(optional)</small><input className="field" value={fields.referral_code} onChange={set("referral_code")} placeholder="e.g. ABC12345" /></label>
...
export function OAuthCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (!code) {
      setError("Login did not complete — missing authorization code.");
      setBusy(false);
      return;
    }
    api.exchangeOAuthCode(code)
      .then(() => refreshUser())
      .then(() => { if (!cancelled) navigate("/app", { replace: true }); })
      .catch((err) => {
        if (!cancelled) setError(err instanceof ApiError ? err.message : "OAuth sign-in failed. Please try again.");
      })
  ...
  return <div className="auth-page"><div className="auth-card">
    {error ? <><h1>Login failed</h1><p className="auth-error">{error}</p><Link className="primary-btn" style={{ display: "block", textAlign: "center", textDecoration: "none" }} to="/login">Back to login</Link></>
      : <p className="auth-loading">{busy ? "Finishing sign-in…" : "Redirecting…"}</p>}
  </div></div>;
}
