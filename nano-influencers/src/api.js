...
// Production note: bearer tokens remain in browser storage for compatibility
// with the current SPA. A future hardening pass should move the session to
...
export const api = {
  async register({ email, password, full_name, referral_code }) {
    return request("/auth/register", { method: "POST", auth: false, body: { email, password, full_name, role: "advertiser", referral_code: referral_code || null } });
  },
  ...
  async initiateDeposit(amount_ngn) { return request("/wallet/deposit/initialize", { method: "POST", body: { amount_ngn } }); },
