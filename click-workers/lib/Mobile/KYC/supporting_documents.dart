...
class _SupportingDocumentsState extends State<SupportingDocuments> {
  ...
  static const idTypes = ['National ID', 'Driver’s License', 'International Passport', 'Voter’s Card', 'Student ID'];
  ...
  Future<void> _continue() async {
    if (idType == 'Select ID Type' || selectedFile?.bytes == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Select an ID type and upload your document')));
      return;
    }
    ...
    try {
      ...
      await ApiClient.instance.uploadToPresignedUrl(upload['upload_url'] as String, file.bytes!, contentType: upload['content_type'] as String);
  ...
  Widget build(BuildContext context) => Scaffold(
    appBar: AppBar(title: const Text('KYC Verification'), backgroundColor: Colors.white), backgroundColor: const Color(0xffeeeeee),
    body: SingleChildScrollView(padding: EdgeInsets.all(5.w), child: Card(elevation: 5, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)), child: Padding(padding: EdgeInsets.all(5.w), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      const Text('Supporting Documents', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), SizedBox(height: 2.h), const Text('Select the type of identification you will use.'), SizedBox(height: 1.h),
      DropdownButtonFormField<String>(value: idType == 'Select ID Type' ? null : idType, hint: const Text('Select ID Type'), isExpanded: true, decoration: const InputDecoration(border: OutlineInputBorder()), items: idTypes.map((v) => DropdownMenuItem(value: v, child: Text(v))).toList(), onChanged: (v) => setState(() => idType = v ?? idType)),
