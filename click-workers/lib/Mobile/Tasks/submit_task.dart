...
class _SubmitTaskState extends State<SubmitTask> {
  ...
  Widget uploadCard() => SizedBox(width: 90.w, child: Card(elevation: 6, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)), color: Colors.white, child: Padding(padding: const EdgeInsets.all(15), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    const Text('Upload Evidence of Completion', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)), SizedBox(height: 3.h), InkWell(onTap: pickFile, child: Container(width: 80.w, padding: const EdgeInsets.all(20), decoration: BoxDecoration(color: Colors.white, border: Border.all(color: Colors.grey.shade300), borderRadius: BorderRadius.circular(16)), child: Column(children: [SizedBox(height: 3.h), const Icon(Icons.cloud_upload_sharp, size: 48, color: Colors.black), SizedBox(height: 2.h), const Text('Drop files here or click to upload', style: TextStyle(fontWeight: FontWeight.bold)), SizedBox(height: 1.h), const Text('Supports PNG, JPG, MP4 (Max 7MB)', style: TextStyle(color: Colors.grey, fontSize: 12)), SizedBox(height: 3.h)])))
  ]))));

  Widget formCard() => SizedBox(width: 90.w, child: Card(elevation: 6, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)), color: Colors.white, child: Padding(padding: const EdgeInsets.all(15), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    buildSelectedFilesPreview(), const SizedBox(height: 16), const Text('Task URL', style: TextStyle(fontWeight: FontWeight.bold)), const SizedBox(height: 8), TextField(controller: _urlController, decoration: InputDecoration(hintText: 'http://', hintStyle: const TextStyle(color: Color(0xff6b7280)), border: OutlineInputBorder(borderRadius: BorderRadius.circular(8)), contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10)))
  ])));
  ...
  Future<List<String>> _uploadSelectedFiles() async {
    ...
    for (final file in _selectedFiles) {
      ...
      await ApiClient.instance.uploadToPresignedUrl(presigned['upload_url'] as String, bytes, contentType: presigned['content_type'] as String);
      keys.add(presigned['file_key'] as String);
    }
    return keys;
  }

  Widget submitButton() => SizedBox(width: 90.w, height: 7.h, child: ElevatedButton(
    ...
    onPressed: _submitting ? null : () async {
      ...
      try {
        ...
        await ApiClient.instance.submitTask(widget.taskID, proofKeys, proofLink: _urlController.text.trim().isEmpty ? null : _urlController.text.trim());
        if (!mounted) return;
        ...
        if (mounted) { setState(() => _submitting = false); ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.toString()))); }
      }
    },
  ...
  static Widget _buildSubmissionItem({required String title, required String date, required String comment, required String status, required Color statusColor, required String photoUrl}) => Container(
    padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(12)), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Container(width: 50, height: 50, margin: const EdgeInsets.only(right: 12), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(8)), child: photoUrl.isEmpty ? const Icon(Icons.link, color: Colors.grey) : Image.network(photoUrl, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const Icon(Icons.broken_image))),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [Text(title, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)), Text(status, style: TextStyle(color: statusColor, fontSize: 12, fontWeight: FontWeight.bold))]), const SizedBox(height: 4), Text('Submitted $date', style: const TextStyle(color: Colors.grey, fontSize: 12)), const SizedBox(height: 6), Text(comment)]))
    ]));

  @override
  Widget build(BuildContext context) => Scaffold(
    backgroundColor: const Color(0xFFF7F7F7), appBar: AppBar(title: const Text('Task Submission', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)), centerTitle: false, backgroundColor: Colors.white, leading: IconButton(icon: const Icon(Icons.arrow_back_ios_new), onPressed: () => Navigator.of(context).pop()), actions: [IconButton(onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const Notifications())), icon: const Icon(Icons.notifications_rounded, color: Colors.black))],
    body: SingleChildScrollView(child: Center(child: FutureBuilder<List<Map<String, dynamic>>>(future: getPreviousSubmissions(), builder: (context, snapshot) {
      if (snapshot.connectionState == ConnectionState.waiting) return const CircularProgressIndicator(color: Colors.black);
      final submissions = snapshot.data ?? [];
      return Column(children: [SizedBox(height: 2.h), uploadCard(), SizedBox(height: 2.h), formCard(), SizedBox(height: 2.h), SizedBox(width: 90.w, child: Card(elevation: 6, color: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)), child: Padding(padding: const EdgeInsets.all(15), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      ]))), SizedBox(height: 2.h), submitButton(), SizedBox(height: 2.h)]);
    }))),
  );
}
