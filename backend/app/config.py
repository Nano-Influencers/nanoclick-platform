...
class Settings(BaseSettings):
    ...
    PASSWORD_RESET_URL: str = "http://localhost:5173/reset-password.html"
    ...
    def validate_production_security(self):
        ...
        for name, value in (("OAUTH_REDIRECT_BASE", self.OAUTH_REDIRECT_BASE), ("OAUTH_WEB_REDIRECT_URL", self.OAUTH_WEB_REDIRECT_URL), ("PAYSTACK_CALLBACK_URL", self.PAYSTACK_CALLBACK_URL), ("PASSWORD_RESET_URL", self.PASSWORD_RESET_URL)):
        ...
        if not redirects or any(not url.startswith("https://") for url in redirects):
            raise ValueError("OAUTH_ALLOWED_WEB_REDIRECTS must contain only HTTPS redirects in production")
        ...
        if not self.SMTP_HOST or not self.SMTP_FROM_EMAIL:
            raise ValueError("SMTP_HOST and SMTP_FROM_EMAIL must be configured in production")
        return self
