...
from app.services.storage import compute_image_hash, validate_uploaded_object
...
async def _check_visibility(task_id: uuid.UUID, worker: User, db: AsyncSession) -> None:
    targeting_r = await db.execute(select(CampaignTargeting).where(CampaignTargeting.campaign_id == select(Task.campaign_id).where(Task.id == task_id).scalar_subquery()))
...
async def _validate_proofs(proof_keys: list[str], worker_id: uuid.UUID) -> None:
    for key in proof_keys:
        try:
            validate_uploaded_object(key, f"proofs/{worker_id}")
        except ValueError as exc:
            raise HTTPException(400, str(exc)) from exc


@router.post("/{task_id}/resubmit", response_model=SubmissionResponse)
async def resubmit_task(task_id: uuid.UUID, body: SubmissionCreate, current_user: User = Depends(require_worker), db: AsyncSession = Depends(get_db)):
    ...
    sub_r = await db.execute(select(Submission).where(Submission.task_id == task_id, Submission.worker_id == current_user.id, Submission.status.in_(["queried", "rejected"])).order_by(Submission.submitted_at.desc()).limit(1).with_for_update())
    ...
    acceptance_r = await db.execute(select(TaskAcceptance).where(TaskAcceptance.id == submission.acceptance_id, TaskAcceptance.task_id == task_id, TaskAcceptance.worker_id == current_user.id).with_for_update())
