...
async def approve_campaign_lifecycle(
    ...
    task_result = await db.execute(
        select(Task).where(Task.campaign_id == campaign_id).with_for_update()
    )
    ...
    return {
        "message": "Campaign approved and tasks are now live",
        "campaign_id": str(campaign_id),
        "tasks_activated": len(tasks),
    }


@router.post("/submissions/{submission_id}/approve")
async def approve_submission_lifecycle(
    ...
    click_points = calculate_click_points(
        cw_task_category=task.cw_task_category,
        worker_pay_kobo=task.pay_kobo,
        is_urgent=task.is_urgent,
        submitted_at=sub.submitted_at,
    )
    await wallet_service.release_escrow_to_worker(
        db=db,
        advertiser_id=campaign.owner_id,
        worker_id=sub.worker_id,
        amount_kobo=task.pay_kobo,
        click_points=click_points,
        task_category=task.cw_task_category,
    ...
    return {
        ...
        "amount_ngn": task.pay_kobo / 100,
        "campaign_slots_filled": campaign.slots_filled,
        "campaign_slots_total": campaign.slots_total,
